...
public abstract class PullToRefreshStickyAdapterViewBase<T extends StickyListHeadersListView>
	...
	private boolean mNeedNextPage = false;
	...
	public PullToRefreshStickyAdapterViewBase(Context context) {
		super(context);
		mRefreshableView.setOnScrollListener(this);
	}

	public PullToRefreshStickyAdapterViewBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		mRefreshableView.setOnScrollListener(this);
	}

	public PullToRefreshStickyAdapterViewBase(Context context, Mode mode) {
		super(context, mode);
		mRefreshableView.setOnScrollListener(this);
	}

	public PullToRefreshStickyAdapterViewBase(Context context, Mode mode, AnimationStyle animStyle) {
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
		...
		mNeedNextPage = (totalItemCount > 0) && (firstVisibleItem + visibleItemCount >= totalItemCount - 2)
				&& (visibleItemCount != totalItemCount);

		if (mMode == Mode.PULL_FROM_END || mMode == Mode.BOTH) {
			if (mNeedNextPage && !mIsFirstLoading && mLastItemVisible && !isRefreshing() && isLastItemVisible()) {
	...
	public final void removeEmptyView(View newEmptyView) {
		FrameLayout refreshableViewWrapper = getRefreshableViewWrapper();
		...
		if (null != newEmptyView) {
			...
			refreshableViewWrapper.removeView(newEmptyView);
		}

		if (mRefreshableView instanceof EmptyViewMethodAccessor) {
			((EmptyViewMethodAccessor) mRefreshableView).setEmptyViewInternal(null);
		} else {
			mRefreshableView.setEmptyView(null);
		}
		mEmptyView = null;
	}
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			if (lastVisiblePosition == lastItemPosition) {
				...
				final View lastVisibleChild = mRefreshableView.getListChildAt(childIndex);
