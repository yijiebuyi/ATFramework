...
import com.callme.platform.widget.stickylistheaders.WrapperViewList.LifeCycleListener;
...
public class StickyListHeadersListView extends FrameLayout {
	...
	public interface OnStickyHeaderOffsetChangedListener {
		 ...
		 *            The view parent
		 * @param header
		 *            The currently sticky header being offset. This header is
		 *            not guaranteed to have it's measurements set. It is
		 *            however guaranteed that this view has been measured,
		 *            therefor you should user getMeasured* methods instead of
		 *            get* methods for determining the view's size.
		 * @param offset
		 *            The amount the sticky header is offset by towards to top
		 *            of the screen.
		 */
        void onStickyHeaderOffsetChanged(StickyListHeadersListView l,
	...
	public interface OnStickyHeaderChangedListener {
		 ...
		 *            The view parent
		 * @param header
		 *            The new sticky header view.
		 * @param itemPosition
		 *            The position of the item within the adapter's data set of
		 *            the item whose header is now sticky.
		 * @param headerId
		 *            The id of the new sticky header.
		 */
        void onStickyHeaderChanged(StickyListHeadersListView l,
	...
	public StickyListHeadersListView(Context context, AttributeSet attrs,
		...
		if (attrs != null) {
			...
			try {
				...
				// scrollbars
				final int scrollBars = a
						.getInt(R.styleable.StickyListHeadersListView_android_scrollbars,
								0x00000200);
				...
				// {
				// mList.setOverScrollMode(a.getInt(R.styleable.StickyListHeadersListView_android_overScrollMode,
				// 0));
				// }
				...
				// a.getInt(R.styleable.StickyListHeadersListView_android_requiresFadingEdge,
				// 0);
				...
				// if
				// (a.hasValue(R.styleable.StickyListHeadersListView_android_divider))
				// {
				// mDivider = a
	...
	public boolean isSwipeEnabled() {
		return mList.isSwipeEnabled();
	}

	public boolean isChoiceMode() {
		return mList.isChoiceMode();
	}

	public void setChoiceMode(boolean is) {
		mList.setChoiceMode(is);
	}
	...
	private void updateOrClearHeader(int firstVisiblePosition) {
		...
		final boolean isFirstViewBelowTop = doesListHaveChildren
				...
				&& mList.getChildAt(0).getTop() > (mClippingToPadding ? mPaddingTop
						: 0);
		...
		if (mHeaderOffset == null || mHeaderOffset != offset) {
			...
			if (mOnStickyHeaderOffsetChangedListener != null) {
				mOnStickyHeaderOffsetChangedListener
						.onStickyHeaderOffsetChanged(this, mHeader,
								-mHeaderOffset);
			}
		}
	}
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
			...
			if (isLastItem) {
				...
				if (onLastItemList) {
					isLastItem = true;
					if (mPageListener != null) {
						mPageListener.onLastListItem();
					}
				}
			}

			// 计算可视item为完整显示的item数量
			final int totalHeight = mList.getHeight();
			final int itemHeight = mList.getChildCount() > 0 ? mList
					.getChildAt(0).getHeight() : 0;
			if (itemHeight != 0) {
				visibleItemCount = totalHeight / itemHeight;
			}
			if (visibleItemCount > 0 && totalItemCount > visibleItemCount
					&& firstVisibleItem > visibleItemCount) {
				if (mPageListener != null) {
					mPageListener.onNextPageShown(true);
				}

			} else {
				if (mPageListener != null) {
					mPageListener.onNextPageShown(false);
				}
			}
		}
	...
	public void setOnStickyHeaderOffsetChangedListener(
			OnStickyHeaderOffsetChangedListener listener) {
		mOnStickyHeaderOffsetChangedListener = listener;
	}

	public void setOnStickyHeaderChangedListener(
			OnStickyHeaderChangedListener listener) {
		mOnStickyHeaderChangedListener = listener;
	}

	public void setStickyListViewListener(StickyListViewListener listener) {
	...
	private boolean requireSdkVersion(int versionCode) {
		if (Build.VERSION.SDK_INT < versionCode) {
			Log.e("StickyListHeaders", "Api lvl must be at least "
					+ versionCode + " to call this method");
			return false;
		}
		return true;
	}
	...
	/**
	 * @return true if the fast scroller will always show. False on
