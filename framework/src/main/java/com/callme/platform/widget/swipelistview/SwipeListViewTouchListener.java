...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private float mRightDeleteWidth = ResourcesUtil.getDimension(R.dimen.px140);
	 ...
	 * Unselected choice state in item
	 */
	protected int dismiss(int position, AnimatorListenerAdapter listener) {
		int start = swipeListView.getFirstVisiblePosition();
		int end = swipeListView.getLastVisiblePosition();
		View view = swipeListView.getChildAt(position - start);
		++dismissAnimationRefCount;
		if (position >= start && position <= end) {
			performDismiss(view, position, false, listener);
	...
	protected boolean getSelected(int position) {
		return checked.get(position);
	}

	protected void setSelected(int position, boolean isChecked) {
		checked.set(position, isChecked);
	}

	protected void unSeleccted(int position) {
		...
		animate(view).translationX(moveTo).alpha(alpha)
				...
				.setListener(new AnimatorListenerAdapter() {
					...
					public void onAnimationEnd(Animator animation) {
						if (swap) {
							closeOpenedItems();
							performDismiss(view, position, true, null);
						}
						resetCell();
					}
				});
					...
					public void onAnimationEnd(Animator animation) {
						...
						if (swap) {
							...
							if (aux) {
								...
								// 让当前item的front view不能有点击态
								view.setBackgroundResource(R.color.common_white_bg_color);
...
//								view.setBackgroundResource(R.drawable.white_grey_selector);
	...
	public OnScrollListener makeScrollListener() {
		return new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView absListView,
				...
				OnScrollListener listener = swipeListView
						.getCustomScrollListener();
				if (listener != null) {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				final int totalHeight = swipeListView.getHeight();
				final int itemHeight = swipeListView.getChildCount() > 0 ? swipeListView
				...
				if (visibleItemCount > 0 && totalItemCount > visibleItemCount
					...
					swipeListView.onNextPageShown(true);

				} else {
					swipeListView.onNextPageShown(false);
				}

				// LogUtil.d(TAG, "visibleItem:" + visibleItemCount
				// + " totalItem:" + totalItemCount + " firstVisible:"
				// + firstVisibleItem);
			}
		};
	}
	...
	boolean closeOtherItems(int pos) {
	...
	/**
	 * @see View.OnTouchListener#onTouch(View,
	 *      MotionEvent)
	 */
	@Override
	public boolean onTouch(View view, MotionEvent motionEvent) {
		if (!swipeListView.isChoiceMode() && !isSwipeEnabled()) {
			return false;
		}
		...
		case MotionEvent.ACTION_DOWN: {
			...
			for (int i = 0; i < childCount; i++) {
				...
				boolean allowSwipe = !isListViewMoving()
						&& isSwipeEnabled()
				...
				if (rect.contains(x, y)) {
					if (allowSwipe) {
						boolean hasOpen = closeOtherItems(childPosition);
						if (hasOpen) {
							return false;
						}
						...
						break;
					} else if (swipeListView.isChoiceMode()) {
						setParentView(child);
						setFrontView(child.findViewById(swipeFrontView));
		...
		case MotionEvent.ACTION_UP: {
			...
			if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
				...
				if (swapRight != swipingRight
					...
					swap = false;
				} else swap = !(opened.get(downPosition)
                        && !openedRight.get(downPosition) && !swapRight);
			}
	...
	public void move(float deltaX) {
		...
		if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
			...
			// 防止超界滑动
			if (SwipeListView.SWIPE_MODE_LEFT == swipeMode
					&& swipeCurrentAction == SwipeListView.SWIPE_ACTION_REVEAL) {
				...
				if (deltaX < -mRightDeleteWidth) {
					deltaX = -mRightDeleteWidth;
				}
			}
			setTranslationX(frontView, deltaX);
		}
