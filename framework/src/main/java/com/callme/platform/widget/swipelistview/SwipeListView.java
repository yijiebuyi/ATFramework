...
public class SwipeListView extends ListView {
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			// styled.getInt(R.styleable.SwipeListView_swipeMode,
			// SWIPE_MODE_BOTH);
			...
			swipeCloseAllItemsWhenMoveList = true;
			// styled.getBoolean(
	...
	public OnScrollListener getCustomScrollListener() {
		return mCustomOnScrollListener;
	}
	 ...
	 */
	public void dismiss(int position, AnimatorListenerAdapter l) {
		int height = touchListener.dismiss(position, l);
		if (height > 0) {
	...
	public boolean getSelected(int position) {
		return touchListener.getSelected(position);
	}

	public void setSelected(int position, boolean isChecked) {
		touchListener.setSelected(position, isChecked);
	}

	public void unSelected(int positon) {
		touchListener.unSeleccted(positon);
		onChoiceChanged(positon, getSelected(positon));
	}
	...
	public void dismissSelected() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			int auxHeight = touchListener.dismiss(position, null);
			if (auxHeight > 0) {
				height = auxHeight;
			}
		}
	...
	protected void onItemDismiss(int pos) {
		if (swipeListViewListener != null) {
			swipeListViewListener.onItemDismiss(pos);
		}
	}

	protected void onNextPageShown(boolean shown) {
		if (swipeListViewListener != null) {
			swipeListViewListener.onNextPageShown(shown);
		}
	}
	...
	protected void onClickFrontView(int position) {
		if (position != AdapterView.INVALID_POSITION) {
			...
			ListAdapter listAda = getAdapter();
			if (listAda instanceof AbsSwipeAdapter) {
				AbsSwipeAdapter ada = (AbsSwipeAdapter) listAda;
				if (ada.isChoiceMode()) {
					unSelected(position);
			...
			if (!handled && swipeListViewListener != null) {
	...
	public boolean isSwipeEnabled() {
		return touchListener.isSwipeEnabled();
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (isChoiceMode || isEnabled() && touchListener.isSwipeEnabled()) {
