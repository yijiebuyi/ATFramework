...
public class FlowLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            // 如果已经需要换行
            if ((childWidth + lp.leftMargin + lp.rightMargin + lineWidth > width) || ((mMaxInLine > 0) && lineCount >= mMaxInLine)) {
                lineCount = 0;
                row++;
                if (isMaxLine(row)) {
                    break;
    ...
    private boolean isMaxLine(int row) {
        return mMaxLine > 0 && mMaxLine == row;

    }

    private boolean isMaxQuantity(int quantity) {
        return mMaxChild > 0 && mMaxChild <= quantity;
