...
public abstract class AbsSwipeAdapter<B, H> extends BaseAdapter implements
    ...
    protected View createItem(final int position) {
        if (mIsChoiceMode || mListView != null
            ...
            LinearLayout frontView = templateView
                    .findViewById(R.id.swipe_front);
            LinearLayout backView = templateView
                    .findViewById(R.id.swipe_back);
            if (mIsChoiceMode) {
                backView.setVisibility(View.GONE);
            }
            FrameLayout contentFrame = frontView
                    .findViewById(R.id.content);
            // 滑动删除list item的自定义区域
            View contentView = createContentItem(position);
            contentFrame.addView(contentView);
            TextView delete = templateView.findViewById(R.id.delete);
            ImageView checkBtn = templateView
...
//		}
    ...
    public AbsSwipeAdapter(Context context,
                           PullToRefreshSwipeListView listView, String url, RequestParams param) {
        this(context);
        if (null == listView) {
            throw new NullPointerException("listview is null");
        }
...
//		soundListener.addSoundEvent(State.REFRESHING, R.raw.refresh);
        ...
        if (!TextUtils.isEmpty(mUrl) && mParams != null
                && mListView instanceof PullToRefreshSwipeListView) {
    ...
    protected void doRequest() {
        ...
        if (!TextUtils.isEmpty(mRequestId)) {
            HttpHandler httpHandler = HttpUtil.mHandlerMap.get(mRequestId);
            if (httpHandler != null) {
                httpHandler.cancel();
                HttpUtil.mHandlerMap.remove(mRequestId);
            }
        }
        mParams.put("page", mCurrentPage);
        mCurrentPage++;
        mRequestId = CmHttpUtil.getInstance(mContext).post(mUrl, mParams,
                new CmRequestListener<JSONObject>(mContext) {
                    ...
                    public void onSuccess(JSONObject response) {
                        ...
                        try {
                            PageBean<Object> bean = new Gson().fromJson(response.toString(),
                                    new TypeToken<PageBean<Object>>() {
                                    }.getType());
                            json = bean.rows;
                        } catch (Exception e) {
                            e.printStackTrace();
...
//                        }
    ...
    protected void addEmptyView() {
        if (mEmptyView == null) {
            ...
            ImageView icon = mEmptyView
                    .findViewById(R.id.empty_icon);
            TextView title = mEmptyView
                    .findViewById(R.id.empty_title);
            TextView tip = mEmptyView.findViewById(R.id.empty_tip);
            if (mEmptyIcon > 0) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            setDeleteListener(delete, position);
            ImageView checkBtn = view.findViewById(R.id.checkbox);
            setCheckListener(checkBtn, position);
        }
