...
public class LazyViewPager extends ViewGroup {
    ...
    private final Map<Integer, ItemInfo> mItems = new HashMap<Integer, ItemInfo>();
    ...
    // 子控件按内容自适应高度，不是靠父控件指定
    private boolean mNeedWrapContent = false;
    ...
    public void setAdapter(PagerAdapter adapter) {
        if (mAdapter != null) {
            ...
            // }
            Iterator<Integer> it = mItems.keySet().iterator();
            while (it.hasNext()) {
                ItemInfo ii = mItems.get(it.next());
                mAdapter.destroyItem(this, ii.position, ii.object);
            }
    ...
    public void setCurrentItem(int item) {
        mPopulatePending = false;
        setCurrentItemInternal(item, !mFirstLayout && isAllowScroll, false);
    }
    ...
    public View getCurrentView() {
        return (View) mAdapter.instantiateItem(this, mCurItem);
    }
    ...
    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always,
        ...
        if (item > (mCurItem + pageLimit) || item < (mCurItem - pageLimit)) {
            ...
            Iterator<Integer> it = mItems.keySet().iterator();
            while (it.hasNext()) {
                mItems.get(it.next()).scrolling = true;
            }
        }
    ...
    ArrayList<OnPageChangeListener> listenerList = new ArrayList<OnPageChangeListener>();
    ...
    public void setOnPageChangeListener(OnPageChangeListener listener) {
        ...
        OnPageChangeListener pageListener = new OnPageChangeListener() {
            ...
            @Override
            public void onPageSelected(int position) {
                for (OnPageChangeListener temp : listenerList) {
            ...
            public void onPageScrolled(int position, float positionOffset,
                                       int positionOffsetPixels) {
                for (OnPageChangeListener temp : listenerList) {
                    temp.onPageScrolled(position, positionOffset,
            ...
            public void onPageScrollStateChanged(int state) {
                for (OnPageChangeListener temp : listenerList) {
                    temp.onPageScrollStateChanged(state);
                }
            }
        };
    ...
    void addNewItem(int position, int index) {
        ...
        // mItems.add(ii);
        // } else {
        // mItems.add(index, mItems.get(index));
        // }
    }
    ...
    void dataSetChanged() {
        ...
        // if (mCurItem == ii.position) {
        // // Keep the current item in the valid range
        // newCurrItem = Math.max(0, Math.min(mCurItem, mAdapter.getCount() -
        // 1));
        // }
        // continue;
        // }
    ...
    void populate() {
        ...
        // }
        // }
        if (mItems.get(mCurItem) == null) {
            addNewItem(mCurItem, mCurItem);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int childHeight = 0;
        if (!mNeedWrapContent) {
            heightMeasureSpec = mChildHeightMeasureSpec;
        }
        for (int i = 0; i < size; ++i) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                child.measure(mChildWidthMeasureSpec, heightMeasureSpec);
                if (!mNeedWrapContent
                        && child.getMeasuredHeight() > childHeight) {
                    childHeight = child.getMeasuredHeight();
                } else if (mNeedWrapContent && mCurItem == i) {
                    childHeight = child.getMeasuredHeight();
                }
            }
        }
    ...
    // 设置viewpager是否可以滑动
    public final void setSrollState(boolean scroll) {
        isAllowScroll = scroll;
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN: {
                if (this.getChildCount() > 1 && !mNeedWrapContent) { // 有内容，多于1个时
                ...
                if (mIsBeingDragged) {
                    ...
                    if (scrollX < leftBound) {
                        ...
                        if (rightBound == lastItemIndex * widthWithMargin) {
                            float over = scrollX - rightBound - 200;
                            // mOverscrollEffect.setPull(over / width);
        ...
        return !(mNeedWrapContent
                && (action & MotionEventCompat.ACTION_MASK) == MotionEvent.ACTION_MOVE
                && !mIsBeingDragged);
    }
    ...
    // private float mOverscrollAnimationDuration=400;
    //
    // /**
    // * @param deltaDistance
    ...
    // invalidateVisibleChilds(mCurItem);
    // }
    //
    ...
    // if (mCurItem == 0 && mOverscroll < 0) {
    // return true;
    // }
    // final boolean isLast = (getAdapter().getCount() - 1) == mCurItem;
