...
import com.callme.platform.util.BitmapOptions;
...
public class ImageLoader {
        ...
        if (instance == null) {
            synchronized (ImageLoader.class) {
                if (instance == null) {
                    ...
                    instance.init(getDefaultConfig(ctx));
                }
            }
        }
        return instance;
    ...
    private static ImageLoaderConfiguration getDefaultConfig(Context ctx) {
    ...
    public void displayImage(String uri, ImageAware imageAware,
        ...
        if (options.getWidth() != 0 || options.getHeight() != 0) {
            ...
            if (options.getWidth() != 0) {
                sb.append(options.getWidth() + "w_");
            }
            if (options.getHeight() != 0) {
                sb.append(options.getHeight() + "h");
            }
            if (options.getImageType() != 0) {
                sb.append(BitmapOptions.getParamByType(options.getImageType()));
            }
            if (options.isPng) {
                sb.append(".png");
            } else if (DevicesUtil.getSystemVersionLevel() < 14) {
                sb.append(".jpg");
            } else {
                sb.append(".webp");
            }
            if (uri.contains("?")) {
                uri = uri.substring(0, uri.lastIndexOf("?")) + sb.toString()
     ...
     * {@link ImageLoadingListener#onLoadingComplete(String, View, Bitmap)}
     * callback}. <br />
     * <b>NOTE:</b> {@link #init(ImageLoaderConfiguration)} method must be
     * called before this method call
     *
     ...
     *                        {@linkplain ImageLoadingListener#onLoadingComplete(String, View, Bitmap)}
     ...
     * @param uri             Image URI (i.e. "http://site.com/image.png",
     *                        "file:///mnt/sdcard/image.png")
     * @param targetImageSize Minimal size for {@link Bitmap} which will be returned.
     ...
     * {@link ImageView ImageView}.
     *
     * @param imageView {@link ImageView ImageView} for which display
     ...
     * problem</a> or not.
     *
     * @param handleSlowNetwork pass <b>true</b> - to use {@link FlushedInputStream} for
